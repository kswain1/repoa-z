...
def test_create_session(player, trainer, trainer_client, muscle):
    ...
    max_effeciency = len(list(filter(lambda r: r >= 0.7 * 15000, readings))) / float(len(readings)) * 100
    submax_effeciency = len(list(filter(lambda r: 0.5 * 15000 <= r < 0.7 * 15000, readings))) / float(len(readings)) * 100
    min_effeciency = len(list(filter(lambda r: 0.0 <= r < 0.5 * 15000, readings))) / float(len(readings)) * 100
